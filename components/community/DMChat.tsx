...
import {
    X, Search, MessageCircle, EllipsisVertical, Reply, Edit3, Trash2,
    CheckCheck, Check, Send, CheckCircle2
} from 'lucide-react';
...
export const DMChat: React.FC<DMChatProps> = ({
    ...
    const unreadCount = unreadDmCounts.get(dmPeerUser.id) || 0;
            ...
            <div className="p-4 border-b border-slate-200 dark:border-white/10 bg-white/80 dark:bg-[#0a0a0c]/80 backdrop-blur-xl">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <img
                            src={dmPeerUser.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(dmPeerUser.name)}`}
                            alt={dmPeerUser.name}
                            className="w-10 h-10 rounded-full"
                        />
                        <div className="overflow-visible">
                            <h2 className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5 overflow-visible">
                                <UserBadge user={dmPeerUser} size="sm" />
                                {dmPeerUser.name}
                                {unreadCount > 0 && (
                                    <span className="ml-2 text-xs font-normal bg-red-500 text-white px-2 py-0.5 rounded-full animate-pulse">
                                        {unreadCount} yangi
                                    </span>
                                )}
                            </h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400">@{dmPeerUser.username}</p>
                        </div>
                    </div>
                </div>
            </div>
