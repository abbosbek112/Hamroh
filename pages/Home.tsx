    ...
    <div className="space-y-24 animate-fade-in pb-12 overflow-x-hidden">
      ...
      {showOnboarding && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={handleCompleteOnboarding}></div>
          <div className="relative w-full max-w-lg bg-white dark:bg-[#1a1a1e] rounded-[2rem] p-8 shadow-2xl border border-slate-200 dark:border-white/10 animate-fade-in-up">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
                {t('home.onboarding.title')}
              </h3>
              <button onClick={handleCompleteOnboarding} className="p-2 bg-slate-100 dark:bg-white/10 rounded-full hover:bg-slate-200">
                <X size={20} />
              </button>
            </div>
            ...
            {onboardingStep === 1 && (
              <div className="space-y-5">
                <p className="text-slate-600 dark:text-slate-300 font-medium">{t('home.onboarding.step1_desc')}</p>
                <div className="grid grid-cols-3 gap-3">
                  {(['uz', 'ru', 'en'] as const).map(lang => (
                    <button
                      key={lang}
                      onClick={() => setLanguage(lang)}
                      className={`py-3 rounded-xl font-bold border transition-all ${language === lang
            ...
            {onboardingStep === 2 && (
              <div className="space-y-4">
                <p className="text-slate-600 dark:text-slate-300 font-medium">{t('home.onboarding.step2_desc')}</p>
                <div className="space-y-3">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('home.onboarding.time_label')}</label>
                  <input
                    type="time"
                    value={onboardingForm.time}
                    onChange={(e) => setOnboardingForm({ ...onboardingForm, time: e.target.value })}
                    className="w-full p-3 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 font-semibold"
                  />
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('home.onboarding.title_label')}</label>
                  <input
                    type="text"
            ...
            {onboardingStep === 3 && (
              <div className="space-y-4">
                <p className="text-slate-600 dark:text-slate-300 font-medium">{t('home.onboarding.step3_desc')}</p>
              ...
              <div className="flex gap-2">
                {onboardingStep > 1 && (
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-white/10 font-bold"
                  >
                ...
                {onboardingStep < 3 && (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl bg-indigo-600 text-white font-bold ${onboardingStep === 2 && !onboardingForm.title.trim()
                      ? 'opacity-60 cursor-not-allowed'
                      : ''
                      }`}
                  >
                    {t('home.onboarding.next')}
                  </button>
                )}
                {onboardingStep === 3 && (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl bg-indigo-600 text-white font-bold flex items-center gap-2 ${savingOnboarding ? 'opacity-60 cursor-not-allowed' : ''
                      }`}
                  >
                    {savingOnboarding ? <Loader2 size={16} className="animate-spin" /> : null}
